...
public class FlowCtlByServiceRule implements IGatewayFlowCtlRule {
    ...
    private static ConcurrentHashMap<String /*serviceId*/, FlowCtlByServiceRule> serviceMap = new ConcurrentHashMap<>();

    //对外暴露获取当前实例的方法，根据当前路径和serviceId获取，提前组装好，不用重复组装
    public static FlowCtlByServiceRule getInstance(String serviceId) {
    ...
    public void doFlowCtlFilter(Rule.FlowCtlConfig flowCtlConfig, String serviceId) {
        ...
        if (flowCtlConfig.getModel().equalsIgnoreCase(FLOW_CTL_MODEL_DISRTIBUTED)) {
            ...
            GuavaCountLimiter guavaCountLimiter = GuavaCountLimiter.getInstance(serviceId, flowCtlConfig, (int) count);
